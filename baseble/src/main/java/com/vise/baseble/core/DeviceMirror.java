...
public class DeviceMirror {
    ...
    private boolean isAutoConnect = false;//是否设置断开自动连接
...
//                        refreshDeviceCache();
        ...
        public void onServicesDiscovered(final BluetoothGatt gatt, final int status) {
            ...
            if (handler != null) {
                ...
                handler.removeMessages(MSG_CONNECT_RETRY);//修复连接失败后再次连接的过程中又连接上了,导致上一个连接无法断开
            }
            if (status == 0) {
    ...
    /**
     *
     * @return 获取设置断开自动连接的状态，true:断开需要重连接 false:断开不需要重连接
     */
    ...
    /**
     * 关闭系统蓝牙的时候需要传入false
     * @param autoConnect 设置断开连接是否需要重新连接,true:需要,false:不需要,默认false
     */
    ...
    /**
     * 设置重连接时断开的设备
     * @param gatt 此时的BluetoothGatt对象
     */
    private synchronized void disconnect(BluetoothGatt gatt) {
        connectState = ConnectState.CONNECT_INIT;
        connectRetryCount = 0;
        if (gatt != null) {
            isActiveDisconnect = true;
    ...
    /**
     * 为了实现单一使能通道,使能通道前必须调用这个初始化方法
     */
    public synchronized void initBindChannel(){
        if (bleCallbackMap != null) {
            bleCallbackMap.clear();
        }
        if (receiveCallbackMap != null) {
    ...
    private synchronized void connectDevice() {
        if (bluetoothLeDevice != null && bluetoothLeDevice.getDevice() != null) {
            ...
            if (bluetoothAdapter != null) {
                BluetoothDevice remoteDevice = bluetoothAdapter.getRemoteDevice(bluetoothLeDevice.getAddress());
                if (remoteDevice != null) {
                    remoteDevice.connectGatt(ViseBle.getInstance().getContext(),false,coreGattCallback);
                }
            }
//            bluetoothLeDevice.getDevice().connectGatt(ViseBle.getInstance().getContext(), false, coreGattCallback);
        }
